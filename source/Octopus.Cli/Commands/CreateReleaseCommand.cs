...
namespace Octopus.Cli.Commands
{
    ...
    public class CreateReleaseCommand : DeploymentCommandBase
    {
            ...
            : base(repositoryFactory, log, fileSystem)
        {
            ...
            options.Add("defaultpackageversion=|packageversion=", "Default version number of all packages to use for this release. Override per-package using --package.",
                v =>
                {
                    try
                    {
                        SemanticVersion.Parse(v);
                        versionResolver.Default(v);
                    }
