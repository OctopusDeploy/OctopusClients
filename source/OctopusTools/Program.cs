...
namespace OctopusTools
{
    public class Program
    {
        ...
        static int PrintError(Exception ex)
        {
            ...
            var octo = ex as WebException;
            if (octo != null)
            {
                Log.Error("Error from Octopus server (HTTP " + ((HttpWebResponse)octo.Response).StatusCode + "): " + octo.Message);
                return -7;
            }

            var http = ex as OctopusHttpApiException;
            if (http != null)
            {
