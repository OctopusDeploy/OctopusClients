using System;
using System.Diagnostics;
using System.IO;
using System.Text;
using System.Threading;

namespace Octopus.Client.E2ETests
{
    public static class SilentProcessRunner
    {
        public static string ExecuteCommand(string executable, string arguments)
        {
            try
            {
                using (var process = new Process())
                {
                    process.StartInfo.FileName = executable;
                    process.StartInfo.Arguments = arguments;
                    process.StartInfo.UseShellExecute = false;
                    process.StartInfo.CreateNoWindow = true;
                    process.StartInfo.RedirectStandardOutput = true;
                    process.StartInfo.RedirectStandardError = true;

                    var output = new StringBuilder();
                    var error = new StringBuilder();

                    using (var outputWaitHandle = new AutoResetEvent(false))
                    using (var errorWaitHandle = new AutoResetEvent(false))
                    {
                        process.OutputDataReceived += (sender, e) =>
                        {
                            if (e.Data == null)
                            {
                                outputWaitHandle.Set();
                            }
                            else
                            {
                                output.AppendLine(e.Data);
                            }
                        };

                        process.ErrorDataReceived += (sender, e) =>
                        {
                            if (e.Data == null)
                            {
                                errorWaitHandle.Set();
                            }
                            else
                            {
                                error.AppendLine(e.Data);
                            }
                        };

                        process.Start();

                        process.BeginOutputReadLine();
                        process.BeginErrorReadLine();

                        process.WaitForExit();
                        outputWaitHandle.WaitOne();
                        errorWaitHandle.WaitOne();

                        var fullOutput = output.ToString();

                        if (!string.IsNullOrWhiteSpace(error.ToString()))
                        {
                            fullOutput = "ERROR: " + Environment.NewLine + Environment.NewLine + error;
                        }

                        if (process.ExitCode != 0)
                        {
                            var logFile = Path.GetTempFileName();
                            File.WriteAllText(logFile, fullOutput);

                            throw new Exception("The return code was: " + process.ExitCode + "\r\n\r\nAdditional information is available at: " + logFile);
                        }

                        return fullOutput;
                    }
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Error when attempting to execute {executable}: {ex.Message}", ex);
            }
        }
    }
}
