    ...
    public class OctopusAsyncRepository : IOctopusAsyncRepository
    {
        ...
        async Task<SpaceRootResource> LoadSpaceRootDocumentInner()
        {
            ...
            return SpaceContext.SpaceIds.Any() ?
                await Client.Get<SpaceRootResource>((await loadRootResource.Value.ConfigureAwait(false)).Link("SpaceHome"), new { spaceId = SpaceContext.SpaceIds.Single() }).ConfigureAwait(false)
                : null;
        }
        ...
        async Task<SpaceResource> TryGetDefaultSpace()
        {
            try
            {
                var currentUser = await Client.Get<UserResource>((await loadRootResource.Value.ConfigureAwait(false)).Links["CurrentUser"]).ConfigureAwait(false);
